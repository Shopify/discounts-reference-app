...
import { reactExtension, useApi, BlockStack, FunctionSettings, Section, Text, Form, NumberField, Box, InlineStack, Heading, } from '@shopify/ui-extensions-react/admin';
...
function PercentageField({ label, defaultValue, value, onChange, name, }) {
    return (<Box>
      <BlockStack gap="base">
        <NumberField label={label} name={name} value={Number(value)} defaultValue={String(defaultValue)} onChange={value => onChange(String(value))} suffix="%"/>
      </BlockStack>
    </Box>);
}
...
function App() {
    const { loading, applyExtensionMetafieldChange, i18n, initialPercentages, onPercentageValueChange, percentages, resetForm, } = useExtensionData();
    return (<>
      {loading ? (<Text>Loading...</Text>) : (<FunctionSettings onSave={applyExtensionMetafieldChange}>
            ...
            <Section>
              <BlockStack gap="base">
                <InlineStack blockAlignment="center">
                  ...
                  <PercentageField value={String(percentages.product)} defaultValue={String(initialPercentages.product)} onChange={value => onPercentageValueChange('product', value)} label={i18n.translate('percentage.Product')} name="product"/>
                </InlineStack>
                <InlineStack blockAlignment="center">
                  <Box maxInlineSize="66%">
                    <Text>{i18n.translate('percentage.descriptionOrder')}</Text>
                  </Box>
                  <PercentageField value={String(percentages.order)} defaultValue={String(initialPercentages.order)} onChange={value => onPercentageValueChange('order', value)} label={i18n.translate('percentage.Order')} name="order"/>
                </InlineStack>
                <InlineStack blockAlignment="center">
                  ...
                  <PercentageField value={String(percentages.shipping)} defaultValue={String(initialPercentages.shipping)} onChange={value => onPercentageValueChange('shipping', value)} label={i18n.translate('percentage.Shipping')} name="shipping"/>
                </InlineStack>
              </BlockStack>
            </Section>
          </Form>
...
function useExtensionData() {
    ...
    useEffect(() => {
        async function fetchInitialData() {
            setLoading(true);
            const config = parsePercentageMetafield(savedMetafields.find(metafield => metafield.key === 'function-configuration')?.value);
